...
pub enum PhaseSource {
    MinMagnitude, // Phase from signal with needed magnitude (depends on algorithm)
    Input1,       // Always use phase from first input
    Input2,       // Always use phase from second input
}

#[derive(Debug, Clone, Copy)]
pub enum StftMode {
    Single,    // Regular STFT
    Multi,     // Multi-resolution STFT
}
...
pub fn parse_phase_source(phase_str: &str) -> PhaseSource {
    match phase_str.to_lowercase().as_str() {
        ...
        _ => PhaseSource::MinMagnitude, // "auto" or any other value
    }
}
...
pub fn stft(signal: &[f32], win: &[f32], hop: usize, fft: &dyn rustfft::Fft<f32>, pb: &ProgressBar) -> Vec<Vec<Complex<f32>>> {
...
pub fn istft(spec: &[Vec<Complex<f32>>], win: &[f32], hop: usize, ifft: &dyn rustfft::Fft<f32>) -> Vec<f32> {
    ...
    // Pre-compute window squares for normalization
